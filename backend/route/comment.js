...
router.get('/comment/:menu_id', async (req, res) => {
    ...
    console.log("/comment/:menu_id in comment.js --> ", menu_id)
...
router.post("/addComment", isLoggedIn, async function (req, res, next) {
        ...
        try {
            let [comment] = await conn.query("INSERT INTO comments (detail, comment_by_id, menu_id) VALUES (?,?,?)", [detail, user_id, menu_id]);

            console.log("addComment in comment.js :", comment)
            await conn.commit();

            let [comment_all] = await pool.query("SELECT *, DATE_FORMAT(comment_date, '%d-%m-%y  (%H:%i)') as comment_date FROM comments " +
            "join users on (comments.comment_by_id = users.user_id) " +
            "WHERE menu_id =?", [menu_id])
